...
export const useGameSetupStore = defineStore('gameSetup', () => {
  const remainingHand = ref<string[]>([])
  const contract = ref<ContractCode>('GARDE')
  const playerIndex = ref(0)
  const takerIndex = ref(0)
  const partnerIndex = ref<number | null>(null)
  const nextPlayerIndex = ref(0)
  const nSamples = ref(200)
  const seed = ref(0)
  const policy = ref<'expected_score'>('expected_score')
  const currentTrick = reactive<TrickDraft>(createEmptyTrick('current'))
  const completedTricks = ref<TrickDraft[]>([])

  const isSubmitting = ref(false)
  const apiError = ref<string | null>(null)
  const lastRecommendation = ref<MoveRecommendationResponse | null>(null)
  const lastSubmittedPayload = ref<MoveRecommendationRequest | null>(null)

  const selectedCardSet = computed(() => {
    const cards = new Set<string>(remainingHand.value)
    for (const entry of currentTrick.entries) {
      if (entry.card) cards.add(entry.card)
    }
  ...
  function toggleRemainingHandCard(token: string): void {
  ...
  function setEntryCard(target: TrickDraft, index: number, token: string | null): void {
    target.entries[index].card = token
  }

  function setEntryPlayer(target: TrickDraft, index: number, playerIndexValue: number | null): void {
    target.entries[index].playerIndex = playerIndexValue
  }

  function addCompletedTrick(): void {
    completedTricks.value.push(createEmptyTrick(`completed-${completedTricks.value.length + 1}`))
  }

  function removeCompletedTrick(id: string): void {
  ...
  function buildPayload(): MoveRecommendationRequest {
    return {
...
function createEmptyTrick(id: string): TrickDraft {
  return {
    id,
...
function normalizeEntries(entries: TrickEntryDraft[]): Array<{ playerIndex: number; card: string }> {
  return entries
    .filter((entry): entry is { playerIndex: number; card: string } => entry.playerIndex !== null && entry.card !== null)
    .map((entry) => ({ playerIndex: entry.playerIndex, card: entry.card }))
}

function collectEntries(tricks: TrickDraft[]): Array<{ playerIndex: number; card: string }> {
  return tricks.flatMap((trick) => normalizeEntries(trick.entries))
}

function serializeTrick(trick: TrickDraft): string {
